...
def play(args):
    ...
    model_dict = torch.load(os.path.join(ROOT_DIR, 'model_100002.pt'))
    ...
    output_names = ['output']  # 输出名称，如果有多个输出，可以是一个列表
    opset_version = 11  # ONNX 操作集版本，根据需要选择合适的版本
    example_input = torch.randn(1, 762).to(env.device).half()
    # 导出模型
    # torch.onnx.export(
    #     policy,
    #     example_input,
    ...
    #     dynamic_axes={'input': {0: 'batch_size'},  # 动态轴设置，如果有批处理维度
    #                   'output': {0: 'batch_size'}}
    # )
    # dummy_input = torch.randn(1, 762)  # 根据实际情况调整形状

    # 导出 body 模型为 ONNX 格式
    # body_onnx_path = '/home/jjp/issac/masterV5-LocomotionWithNP3O/LocomotionWithNP3O/' + 'body_latest.onnx'
    # torch.onnx.export(policy, dummy_input, body_onnx_path, opset_version=11)

    # torch.save(policy,'11-9.pt',)
    ...
    for i in range(num_frames):
        ...
        print("obs:",obs[:,0:48])
        print("last:",obs[:,282:330])
        print("last222:",obs[:,714:762])
